...
using Poco::SQL::SQLite::Notifier;
using Poco::Nullable;
using Poco::Tuple;
using Poco::Any;
using Poco::AnyCast;
...
using Poco::SQL::SQLite::ConstraintViolationException;
using Poco::SQL::SQLite::ParameterCountMismatchException;
using Poco::Int32;
using Poco::Int64;
...
void SQLiteTest::testInMemory()
{
	...
	Session mem (Poco::SQL::SQLite::Connector::KEY, ":memory:");
	assert (Poco::SQL::SQLite::Utility::fileToMemory(mem, "dummy.db"));
	...
	Session dsk (Poco::SQL::SQLite::Connector::KEY, "dsk.db");
	assert (Poco::SQL::SQLite::Utility::memoryToFile("dsk.db", mem));
...
void SQLiteTest::testRowIteratorLimit()
{
	...
	int count = 0;
	Statement stmt = (ses << "select * from Vectors", Poco::SQL::Keywords::limit(1));
	while (!stmt.done())
	{
		stmt.execute(false);
...
void SQLiteTest::testSessionTransaction()
{
	...
	Session local (Poco::SQL::SQLite::Connector::KEY, "dummy.db");

	...
	std::size_t actualSize)
{
	...
	if (_pTypeInfo)
	{
		...
		if (found) 
			if (sizeof(SQLINTEGER) == sizeof(Int32))
				reinterpret_cast<Int32&>(colSize) = tmp;
			else
			if (sizeof(SQLINTEGER) == sizeof(Int64))
				reinterpret_cast<Int64&>(colSize) = tmp;
			else
				throw InvalidArgumentException("Invalid SQLINTEGER size");
...
//On Linux, PostgreSQL driver segfaults on SQLGetDescField, so this is disabled for now
		...
		SQLHDESC hIPD = 0;
		if (!Utility::isError(Poco::SQL::ODBC::SQLGetStmtAttr(_rStmt, SQL_ATTR_IMP_PARAM_DESC, &hIPD, SQL_IS_POINTER, 0)))
		{
			SQLUINTEGER sz = 0;
			if (!Utility::isError(Poco::SQL::ODBC::SQLGetDescField(hIPD, (SQLSMALLINT)pos + 1, SQL_DESC_LENGTH, &sz, SQL_IS_UINTEGER, 0)) &&
				sz > 0)
			{
				size = sz;
			}
...
void Binder::setParamSetSize(std::size_t length)
{
	if (0 == _paramSetSize)
	{
		if (Utility::isError(Poco::SQL::ODBC::SQLSetStmtAttr(_rStmt, SQL_ATTR_PARAM_BIND_TYPE, SQL_PARAM_BIND_BY_COLUMN, SQL_IS_UINTEGER)) ||
			Utility::isError(Poco::SQL::ODBC::SQLSetStmtAttr(_rStmt, SQL_ATTR_PARAMSET_SIZE, (SQLPOINTER) length, SQL_IS_UINTEGER)))

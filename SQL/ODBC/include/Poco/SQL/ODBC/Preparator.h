...
#ifndef SQL_ODBC_Preparator_INCLUDED
#define SQL_ODBC_Preparator_INCLUDED
...
public:
	...
	void prepare(std::size_t pos, const Poco::SQL::BLOB& val);
		/// Prepares a BLOB.

	void prepare(std::size_t pos, const std::vector<Poco::SQL::BLOB>& val);
		/// Prepares a BLOB vector.

	void prepare(std::size_t pos, const std::deque<Poco::SQL::BLOB>& val);
		/// Prepares a BLOB deque.

	void prepare(std::size_t pos, const std::list<Poco::SQL::BLOB>& val);
		/// Prepares a BLOB list.

	void prepare(std::size_t pos, const Poco::SQL::CLOB& val);
		/// Prepares a CLOB.

	void prepare(std::size_t pos, const std::vector<Poco::SQL::CLOB>& val);
		/// Prepares a CLOB vector.

	void prepare(std::size_t pos, const std::deque<Poco::SQL::CLOB>& val);
		/// Prepares a CLOB deque.

	void prepare(std::size_t pos, const std::list<Poco::SQL::CLOB>& val);
		/// Prepares a CLOB list.

	void prepare(std::size_t pos, const Poco::SQL::Date& val);
		/// Prepares a Date.

	void prepare(std::size_t pos, const std::vector<Poco::SQL::Date>& val);
		/// Prepares a Date vector.

	void prepare(std::size_t pos, const std::deque<Poco::SQL::Date>& val);
		/// Prepares a Date deque.

	void prepare(std::size_t pos, const std::list<Poco::SQL::Date>& val);
		/// Prepares a Date list.

	void prepare(std::size_t pos, const Poco::SQL::Time& val);
		/// Prepares a Time.

	void prepare(std::size_t pos, const std::vector<Poco::SQL::Time>& val);
		/// Prepares a Time vector.

	void prepare(std::size_t pos, const std::deque<Poco::SQL::Time>& val);
		/// Prepares a Time deque.

	void prepare(std::size_t pos, const std::list<Poco::SQL::Time>& val);
...
private:
		...
		/// Utility function to prepare Any and DynamicAny.
	{
		...
		switch (col.type())
		{
			...
			{
				typedef Poco::SQL::BLOB::ValueType CharType;
				if (pVal)
			...
			{
				typedef Poco::SQL::CLOB::ValueType CharType;
				if (pVal)
...
inline void Preparator::prepare(std::size_t pos, const Poco::SQL::BLOB&)
{
	prepareVariableLen<Poco::SQL::BLOB::ValueType>(pos, SQL_C_BINARY, maxDataSize(pos), DT_UCHAR);
}


inline void Preparator::prepare(std::size_t pos, const std::vector<Poco::SQL::BLOB>& val)
{
	prepareCharArray<char, DT_UCHAR_ARRAY>(pos, SQL_C_BINARY, maxDataSize(pos), val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::deque<Poco::SQL::BLOB>& val)
{
	prepareCharArray<char, DT_UCHAR_ARRAY>(pos, SQL_C_BINARY, maxDataSize(pos), val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::list<Poco::SQL::BLOB>& val)
{
	prepareCharArray<char, DT_UCHAR_ARRAY>(pos, SQL_C_BINARY, maxDataSize(pos), val.size());
}


inline void Preparator::prepare(std::size_t pos, const Poco::SQL::CLOB&)
{
	prepareVariableLen<Poco::SQL::CLOB::ValueType>(pos, SQL_C_BINARY, maxDataSize(pos), DT_CHAR);
}


inline void Preparator::prepare(std::size_t pos, const std::vector<Poco::SQL::CLOB>& val)
{
	prepareCharArray<char, DT_CHAR_ARRAY>(pos, SQL_C_BINARY, maxDataSize(pos), val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::deque<Poco::SQL::CLOB>& val)
{
	prepareCharArray<char, DT_CHAR_ARRAY>(pos, SQL_C_BINARY, maxDataSize(pos), val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::list<Poco::SQL::CLOB>& val)
{
	prepareCharArray<char, DT_CHAR_ARRAY>(pos, SQL_C_BINARY, maxDataSize(pos), val.size());
}


inline void Preparator::prepare(std::size_t pos, const Poco::SQL::Date&)
{
	prepareFixedSize<SQL_DATE_STRUCT>(pos, SQL_C_TYPE_DATE);
}


inline void Preparator::prepare(std::size_t pos, const std::vector<Poco::SQL::Date>& val)
{
	prepareFixedSize<SQL_DATE_STRUCT>(pos, SQL_C_TYPE_DATE, val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::deque<Poco::SQL::Date>& val)
{
	prepareFixedSize<SQL_DATE_STRUCT>(pos, SQL_C_TYPE_DATE, val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::list<Poco::SQL::Date>& val)
{
	prepareFixedSize<SQL_DATE_STRUCT>(pos, SQL_C_TYPE_DATE, val.size());
}


inline void Preparator::prepare(std::size_t pos, const Poco::SQL::Time&)
{
	prepareFixedSize<SQL_TIME_STRUCT>(pos, SQL_C_TYPE_TIME);
}


inline void Preparator::prepare(std::size_t pos, const std::vector<Poco::SQL::Time>& val)
{
	prepareFixedSize<SQL_TIME_STRUCT>(pos, SQL_C_TYPE_TIME, val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::deque<Poco::SQL::Time>& val)
{
	prepareFixedSize<SQL_TIME_STRUCT>(pos, SQL_C_TYPE_TIME, val.size());
}


inline void Preparator::prepare(std::size_t pos, const std::list<Poco::SQL::Time>& val)
{
	prepareFixedSize<SQL_TIME_STRUCT>(pos, SQL_C_TYPE_TIME, val.size());
}

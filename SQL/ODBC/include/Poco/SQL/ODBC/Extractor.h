...
#ifndef SQL_ODBC_Extractor_INCLUDED
#define SQL_ODBC_Extractor_INCLUDED
...
class ODBC_API Extractor: public Poco::SQL::AbstractExtractor
...
public:
	...
	bool extract(std::size_t pos, Poco::SQL::BLOB& val);
		/// Extracts a BLOB.

	bool extract(std::size_t pos, Poco::SQL::CLOB& val);
		/// Extracts a CLOB.

	bool extract(std::size_t pos, std::vector<Poco::SQL::BLOB>& val);
		/// Extracts a BLOB vector.

	bool extract(std::size_t pos, std::deque<Poco::SQL::BLOB>& val);
		/// Extracts a BLOB deque.

	bool extract(std::size_t pos, std::list<Poco::SQL::BLOB>& val);
		/// Extracts a BLOB list.

	bool extract(std::size_t pos, std::vector<Poco::SQL::CLOB>& val);
		/// Extracts a CLOB vector.

	bool extract(std::size_t pos, std::deque<Poco::SQL::CLOB>& val);
		/// Extracts a CLOB deque.

	bool extract(std::size_t pos, std::list<Poco::SQL::CLOB>& val);
		/// Extracts a CLOB list.

	bool extract(std::size_t pos, Poco::SQL::Date& val);
		/// Extracts a Date.

	bool extract(std::size_t pos, std::vector<Poco::SQL::Date>& val);
		/// Extracts a Date vector.

	bool extract(std::size_t pos, std::deque<Poco::SQL::Date>& val);
		/// Extracts a Date deque.

	bool extract(std::size_t pos, std::list<Poco::SQL::Date>& val);
		/// Extracts a Date list.

	bool extract(std::size_t pos, Poco::SQL::Time& val);
		/// Extracts a Time.

	bool extract(std::size_t pos, std::vector<Poco::SQL::Time>& val);
		/// Extracts a Time vector.

	bool extract(std::size_t pos, std::deque<Poco::SQL::Time>& val);
		/// Extracts a Time deque.

	bool extract(std::size_t pos, std::list<Poco::SQL::Time>& val);
...
private:
	...
	bool extractBoundImpl(std::size_t pos, Poco::SQL::BLOB& val);
	bool extractBoundImpl(std::size_t pos, Poco::SQL::CLOB& val);
	...
	bool extractBoundImplContainer(std::size_t pos, std::vector<Poco::SQL::CLOB>& values);
	bool extractBoundImplContainer(std::size_t pos, std::deque<Poco::SQL::CLOB>& values);
	bool extractBoundImplContainer(std::size_t pos, std::list<Poco::SQL::CLOB>& values);
	bool extractBoundImplContainer(std::size_t pos, std::vector<Poco::SQL::BLOB>& values);
	bool extractBoundImplContainer(std::size_t pos, std::deque<Poco::SQL::BLOB>& values);
	bool extractBoundImplContainer(std::size_t pos, std::list<Poco::SQL::BLOB>& values);
	...
	template<typename T>
	bool extractBoundImplLOB(std::size_t pos, Poco::SQL::LOB<T>& val)
	{
		if (isNull(pos)) return false;
	...
	template<typename T>
	bool extractManualLOBImpl(std::size_t pos, Poco::SQL::LOB<T>& val, SQLSMALLINT cType);
		...
		/// Utility function for extraction of Any and DynamicAny.
	{
		...
		switch (column.type())
		{
			...
			{ return extAny<T, Poco::SQL::BLOB>(pos, val); }

			case MetaColumn::FDT_CLOB:
			{ return extAny<T, Poco::SQL::CLOB>(pos, val); }

			case MetaColumn::FDT_DATE:
			{ return extAny<T, Poco::SQL::Date>(pos, val); }

			case MetaColumn::FDT_TIME:
			{ return extAny<T, Poco::SQL::Time>(pos, val); }
...
inline bool Extractor::extractBoundImpl(std::size_t pos, Poco::SQL::BLOB& val)
{
	return extractBoundImplLOB<BLOB::ValueType>(pos, val);
}


inline bool Extractor::extractBoundImpl(std::size_t pos, Poco::SQL::CLOB& val)
{
...
inline bool Extractor::extractBoundImplContainer(std::size_t pos,
	std::vector<Poco::SQL::CLOB>& values)
{
...
inline bool Extractor::extractBoundImplContainer(std::size_t pos,
	std::deque<Poco::SQL::CLOB>& values)
{
	return extractBoundImplContainerLOB(pos, values);
}
...
inline bool Extractor::extractBoundImplContainer(std::size_t pos,
	std::vector<Poco::SQL::BLOB>& values)
{
	return extractBoundImplContainerLOB(pos, values);
}
...
inline bool Extractor::extractBoundImplContainer(std::size_t pos,
	std::deque<Poco::SQL::BLOB>& values)
{
	return extractBoundImplContainerLOB(pos, values);
}

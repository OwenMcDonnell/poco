...
bool Extractor::extract(std::size_t pos, std::string& val)
{
	...
	if (columnType != Poco::SQL::MetaColumn::FDT_STRING && columnType != Poco::SQL::MetaColumn::FDT_BLOB)
		throw MySQLException("Extractor: not a string");

	if (columnType == Poco::SQL::MetaColumn::FDT_BLOB && _metadata.length(pos) > 0 && _metadata.rawData(pos) == NULL)
	{
...
bool Extractor::extract(std::size_t pos, Poco::SQL::BLOB& val)
{
	...
	MetaColumn::ColumnDataType columnType = _metadata.metaColumn(static_cast<Poco::UInt32>(pos)).type();
	if (columnType != Poco::SQL::MetaColumn::FDT_BLOB)
